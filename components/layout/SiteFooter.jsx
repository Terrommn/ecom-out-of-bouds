import Link from "next/link";
import { BrandWordmark } from "@/components/brand/BrandWordmark";

const company = [
  { href: "/acerca", label: "Acerca de Nosotros" },
  { href: "/contacto", label: "Contacto" },
];

const service = [
  { href: "/faq", label: "FAQ" },
  { href: "/envios", label: "Envíos" },
  { href: "/devoluciones", label: "Devoluciones" },
];

const legal = [
  { href: "/terminos", label: "Términos y Condiciones" },
  { href: "/privacidad", label: "Política de Privacidad" },
];

const social = [
  { href: "https://www.instagram.com/outofbounds.mx", label: "Instagram", icon: "ig" },
];

export function SiteFooter() {
  return (
    <footer className="relative z-[1] border-t border-[color:var(--oob-border)] bg-[var(--oob-bg-elevated)] mt-24 pb-12 pt-16">
      <div className="oob-container">
        <div className="mb-12 flex flex-col gap-6 border-b border-[color:var(--oob-border)] pb-12 sm:flex-row sm:items-center sm:justify-between">
          <Link href="/" className="group inline-flex max-w-md shrink-0">
            <BrandWordmark variant="onLight" className="transition-opacity group-hover:opacity-85" />
          </Link>
          <p className="max-w-md text-sm leading-relaxed text-[var(--oob-muted)]">
            Moda de golf con carácter. Diseñado para quienes juegan fuera de los límites.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--oob-gold)] mb-4">
              Empresa
            </p>
            <ul className="space-y-3">
              {company.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-sm text-[var(--oob-cream)]/85 hover:text-[var(--oob-gold)] transition-colors"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--oob-gold)] mb-4">
              Servicio al cliente
            </p>
            <ul className="space-y-3">
              {service.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-sm text-[var(--oob-cream)]/85 hover:text-[var(--oob-gold)] transition-colors"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--oob-gold)] mb-4">
              Legal
            </p>
            <ul className="space-y-3">
              {legal.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-sm text-[var(--oob-cream)]/85 hover:text-[var(--oob-gold)] transition-colors"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--oob-gold)] mb-4">
              Conecta con nosotros
            </p>
            <div className="flex flex-wrap gap-3">
              {social.map((s) => (
                <a
                  key={s.href}
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[color:var(--oob-border)] text-[var(--oob-cream)] hover:border-[var(--oob-gold)] hover:text-[var(--oob-gold)] transition-colors"
                  aria-label={s.label}
                >
                  {s.icon === "ig" && (
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                    </svg>
                  )}
                  {s.icon === "fb" && (
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  )}
                  {s.icon === "x" && (
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg>
                  )}
                </a>
              ))}
            </div>
          </div>
        </div>
        <div className="oob-line-gold mt-14 mb-8 max-w-md" />
        <p className="text-center text-xs text-[var(--oob-muted)]">
          © {new Date().getFullYear()} Out Of Bounds. Todos los derechos reservados.
        </p>
      </div>
    </footer>
  );
}
